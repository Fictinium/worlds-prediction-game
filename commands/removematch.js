...
export default {
  data: new SlashCommandBuilder()
    ...
    .addStringOption(o =>
      o.setName('match_id')
        .setDescription('Match ID to delete (alternative to team/start)')
        .setRequired(false)
    )
    .addStringOption(o =>
      o.setName('team_a')
    ...
    .addStringOption(o =>
      o.setName('start')
        .setDescription('Start datetime ISO8601 (if not using match_id)')
        .setRequired(false)
    )
    ...
    try {
      ...
      if (matchId) {
        ...
        if (!teamAName || !teamBName || !startStr) {
          return interaction.editReply('❌ Provide either `match_id` **or** all of `team_a`, `team_b`, and `start` (ISO8601).');
        }
        ...
        if (!startTime) return interaction.editReply('❌ Invalid `start`. Use ISO8601, e.g. `2025-10-21T17:00:00Z`.');

        match = await Match.findOne({ teamA: teamA._id, teamB: teamB._id, startTime });
        if (!match) {
          return interaction.editReply('❌ No match found with those teams and start time.');
        }
      }
      ...
      return interaction.editReply('❌ Failed to remove match.');
    }
  }
};